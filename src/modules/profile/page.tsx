...
export const ProfilePage = ({ username }: { username?: string }) => {
  ...
  const [activeTab, setActiveTab] = useState<
    "showcase" | "activity" | "achievements" | "skills"
  >("showcase");

  const isOwnProfile =
    !username ||
    (currentUser &&
      (username === currentUser.id || username === currentUser.email));

  useEffect(() => {
    getProfile(username);
    getActivities();
  }, [username, getProfile, getActivities]);
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-6">
              ...
              <div className="flex gap-3 mb-2">
                {isOwnProfile ? (
                  <Button
                    variant="outline"
                    className="gap-2 border-gray-200 dark:border-gray-800 font-bold"
                    onClick={() => setIsEditModalOpen(true)}
                  >
                    <Edit2 size={16} /> Edit Profile
                  </Button>
                ) : (
                  <Button className="gap-2">Follow User</Button>
                )}
                <Button
                  variant="transparent"
            ...
            <Card className="border-none shadow-sm">
              ...
              <CardContent className="space-y-3">
                ...
                {profile.portfolioLinks?.map((link: string, index: number) => (
                  ...
                  >
                    <Globe size={16} /> {new URL(link).hostname}
                  </a>
                ))}
              </CardContent>
            </Card>
                  ...
                  >
                    {profile.projects
                      ...
                      .map((up: any) => (
                        <Card
                          key={up.id}
                          className="group hover:border-blue-200 transition-all cursor-pointer overflow-hidden"
                          onClick={() =>
                            router.push(`/projects/${up.projectId}`)
                          }
                        >
                          <div className="h-32 bg-gray-100 relative">
                            <img
                              src={
                                up.project?.coverImage ||
                                `https://placehold.co/600x400/3b82f6/white?text=${up.project?.title}`
                              }
                              className="w-full h-full object-cover"
                            />
                          ...
                          <CardContent className="p-4">
                            ...
                            <div className="mt-4 flex justify-between items-center">
                              <div className="text-xs text-gray-400">
                                Completed{" "}
                                {new Date(up.completedAt).toLocaleDateString()}
                              </div>
                              <Button
                                variant="transparent"
                                size="sm"
                                className="h-8 text-blue-600 font-bold p-0"
                              >
                                View Project{" "}
                ...
                {activeTab === "activity" && (
                  ...
                  >
                    {activities.length > 0 ? (
                      ...
                      <div className="py-12 text-center text-gray-400 border-2 border-dashed rounded-3xl">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        {Array.from(
                          new Set(
                            profile.projects
                              ?.map((up: any) => up.project?.technologies || [])
                              .flat()
                          )
                        ).length > 0 ? (
                          Array.from(
                            new Set(
                              profile.projects
                                ?.map(
                                  (up: any) => up.project?.technologies || []
                                )
                                .flat()
                            )
                          ).map((skill: any) => (
                            <div
                              key={skill}
                              className="px-4 py-2 bg-white border rounded-xl flex items-center gap-2 shadow-sm"
                            >
                          ...
                          <div className="text-gray-400 italic">
                            No skills acquired yet. Complete projects to earn
                            skills!
                          </div>
                        )}
                      </div>
